    ...
    const fetchWeather = async () => {
      try {
        const response = await fetch(
          `https://api.openweathermap.org/data/2.5/weather?q=Seoul&appid=${process.env.REACT_APP_WEATHER_API_KEY}&units=metric`
        );
        const data = await response.json();
        setStats(prev => ({ ...prev, weather: data }));
      } catch (error) {
      ...
      <Grid container spacing={3}>
        ...
        {stats.weather && (
          <Grid item xs={12} md={4}>
            <Card>
              <CardContent>
                <Typography variant="h6" gutterBottom>현재 날씨</Typography>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                  <img
                    src={`http://openweathermap.org/img/wn/${stats.weather.weather[0].icon}@2x.png`}
                    alt={stats.weather.weather[0].description}
                    style={{ width: 50, height: 50 }}
                  />
                  <Box>
                    <Typography variant="h5">{Math.round(stats.weather.main.temp)}°C</Typography>
                    <Typography variant="body2" color="text.secondary">
                      {stats.weather.weather[0].description}
                    </Typography>
                  </Box>
                </Box>
              </CardContent>
