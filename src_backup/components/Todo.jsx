...
const Todo = () => {
  ...
  const fetchTodos = useCallback(async () => {
    try {
      setLoading(true);
      const data = await getTodos({ filter, sortBy });
      setTodos(data);
      setError(null);
    } catch (err) {
  ...
  const handleDragEnd = async (result) => {
    ...
    try {
      await reorderTodos(items.map(item => item.id));
    } catch (err) {
      toast.error('할 일 순서 변경 중 오류가 발생했습니다.');
      fetchTodos(); // 원래 순서로 복구
    }
  };
                      ...
                      >
                        {editingId === todo.id ? (
                          <input
                            ...
                            onBlur={(e) => handleUpdateTodo(todo.id, { title: e.target.value })}
                            onKeyPress={(e) => {
                              if (e.key === 'Enter') {
