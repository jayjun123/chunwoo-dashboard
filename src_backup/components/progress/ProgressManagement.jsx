...
const ProgressManagement = () => {
  ...
  useEffect(() => {
    if (selectedSite) {
      const fetchProgress = async () => {
        setLoading(true);
        setLoadingMessage('기성현황 데이터를 불러오는 중...');
        try {
  ...
  // 차트 데이터 계산
  const chartData = progressData.reduce((acc, curr) => {
    const date = new Date(curr.date).toLocaleDateString();
    const existingData = acc.find(item => item.date === date);
    
    if (existingData) {
      if (curr.type === '청구') {
        existingData.청구 += Number(curr.amount);
      } else {
        existingData.지급 += Number(curr.amount);
      }
    } else {
      acc.push({
        date,
        청구: curr.type === '청구' ? Number(curr.amount) : 0,
        지급: curr.type === '지급' ? Number(curr.amount) : 0,
      });
    }
    return acc;
  }, []);
                ...
                <Paper sx={{ p: 3, mt: 3 }}>
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={chartData}>
                      ...
                      <Tooltip />
                      <Legend />
                      <Bar dataKey="청구" fill="#8884d8" />
                      <Bar dataKey="지급" fill="#82ca9d" />
                    </BarChart>
                  </ResponsiveContainer>
                </Paper>
