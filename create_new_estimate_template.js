...
async function createNewEstimateTemplate() {
  try {
    ...
    // 수식 설정 (공유 수식 없이 개별 수식으로)
    detailSheet.getCell('F8').formula = '=SUM(F5:F7)';
    detailSheet.getCell('H8').formula = '=SUM(H5:H7)';
    detailSheet.getCell('J8').formula = '=SUM(J5:J7)';
    detailSheet.getCell('L8').formula = '=SUM(L5:L7)';
    
    // 수정된 템플릿 저장
    const buffer = await workbook.xlsx.writeBuffer();
    console.log('✅ 새로운 템플릿 생성 완료:', buffer.length, 'bytes');
    
    // 파이어베이스에 업로드
    const templateRef = ref(storage, 'templates/견적서.xlsx');
    await uploadBytes(templateRef, buffer);
    console.log('✅ 새로운 템플릿 업로드 완료');
    
    console.log('🎉 공유 수식 없는 새로운 견적서 템플릿 생성 완료!');
    
  } catch (error) {
    console.error('❌ 새로운 템플릿 생성 실패:', error);
  }
}

createNewEstimateTemplate();
