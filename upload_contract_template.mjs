// 납품계약서 갑지 템플릿 Firebase Storage 업로드
...
const uploadContractTemplate = async () => {
  try {
    console.log('📤 납품계약서 갑지 템플릿 Firebase Storage 업로드 시작...');
    
    // 파일 경로
    const filePath = path.join(process.cwd(), 'public', '납품계약서 갑지.xlsx');
    ...
    // Firebase Storage 경로
    const storagePath = 'templates/contract_gabji.xlsx';
    ...
    console.log('🎉 납품계약서 갑지 템플릿 Firebase Storage 업로드 완료!');
